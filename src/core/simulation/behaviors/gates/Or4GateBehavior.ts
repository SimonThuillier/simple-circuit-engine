/**
 * OR4 Gate component behavior implementation (4 inputs)
...
import { Or4GateState } from '../../states/gates/Or4GateState';
import { ComponentType } from '../../../types/ComponentType';
import { OrGateBehavior } from './OrGateBehavior';

/**
 * Behavior implementation for OR4 Gate components (4 inputs).
 * With positive activationLogic: OR gate (output high when ANY of 4 inputs is high).
 * With negative activationLogic: NOR gate (output high when ALL 4 inputs are low).
 *
 * @public
 */
export class Or4GateBehavior extends OrGateBehavior {
  override readonly componentType = ComponentType.Or4Gate;

  override createInitialState(component: Component): ComponentState {
    if (component.type !== ComponentType.Or4Gate) {
      throw new Error(`Invalid component type for Or4GateBehavior: ${component.type}`);
    }
    const state = component.config.get('activationLogic') === 'negative' ? 'high' : 'low';
    return new Or4GateState(component.id, state);
  }
