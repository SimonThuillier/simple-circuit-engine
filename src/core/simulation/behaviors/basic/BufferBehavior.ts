/**
 * Buffer component behavior implementation
...
import { BufferState } from '../../states/basic/BufferState';
...
/**
 * Behavior implementation for Buffers components.
 *
 * @public
 */
export class BufferBehavior implements ComponentBehavior {
  readonly componentType = ComponentType.Buffer;
  ...
  createInitialState(component: Component): ComponentState {
    if (component.type !== ComponentType.Buffer) {
      throw new Error(`Invalid component type for BufferBehavior: ${component.type}`);
    }
    const state = component.config.get('activationLogic') === 'negative' ? 'high' : 'low';
    return new BufferState(component.id, state);
  }
