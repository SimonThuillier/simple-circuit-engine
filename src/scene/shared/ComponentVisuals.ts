...
function createPinGroup(componentId: string, pinId: string, label: string): THREE.Group {
  ...
  pinGroup.userData = {
    ...
    enodeId: pinId, // Store actual pin UUID
    label: label,
  };
  ...
  hitbox.userData.hoverCallback = (isHovering: boolean) => {
    if (isHovering) {
      (visual.material as THREE.MeshStandardMaterial).color = new THREE.Color(0x00ff00);
    } else {
      (visual.material as THREE.MeshStandardMaterial).color = new THREE.Color(0x0000ff);
    }
  };
...
export const batteryFactory: ComponentVisualFactory = (component: Component) => {
  ...
  const cathodeGroup = createPinGroup(component.id, component.pins[0]!, 'cathode');
  cathodeGroup.position.set(0, 0, -1);
  cathodeGroup.rotateX(-Math.PI / 2);
  group.add(cathodeGroup);

  // anode (negative pin) group
  const anodeGroup = createPinGroup(component.id, component.pins[1]!, 'anode');
...
export const switchFactory: ComponentVisualFactory = (component: Component) => {
  ...
  const contactorGeometry = new THREE.CylinderGeometry(0.2, 0.12, 1.5, 8, 4, false, 0, Math.PI * 2);
  ...
  // input pin group
  const inputPinGroup = createPinGroup(component.id, component.pins[0]!, 'input');
  ...
  // output pin group
  const outputPinGroup = createPinGroup(component.id, component.pins[1]!, 'output');
