...
import { OrGateVisualFactory } from './OrGateVisualFactory';
 ...
 */
export class Or8GateVisualFactory extends OrGateVisualFactory {
  ...
  protected override readonly transientGeometry = OrGateGeometry(2.6, 7.5, 0.76, 0.4, 16);
  /** Shared transient envelope geometry */
  ...
  override createVisual(component: Component): THREE.Object3D {
    ...
    envelopeMaterial.emissive.setHex(Or8GateVisualFactory.HIGH_COLOR);
    ...
    input1Group.position.set(0.83, 0, 3.2);
    ...
    const input2Group = this.createPinGroup(
      ...
      'input2',
      null,
      new THREE.Euler(0.45, 0, 0)
    );
    input2Group.position.set(0.23, 0, 2.25);
    input2Group.rotateZ(Math.PI / 2);
    input2Group.rotateX(Math.PI);
    group.add(input2Group);

    const input3Group = this.createPinGroup(
      component.id,
      component.pins[3]!,
      'input3',
      null,
    ...
    input4Group.position.set(-0.27, 0, 0.33);
    input4Group.rotateZ(Math.PI / 2);
    input4Group.rotateX(Math.PI);
    group.add(input4Group);

    const input5Group = this.createPinGroup(
      component.id,
      component.pins[5]!,
      'input5',
      null,
      new THREE.Euler(-0.13, 0, 0)
    );
    input5Group.position.set(-0.28, 0, -0.63);
    input5Group.rotateZ(Math.PI / 2);
    input5Group.rotateX(Math.PI);
    group.add(input5Group);

    const input6Group = this.createPinGroup(
      component.id,
      component.pins[6]!,
      'input6',
      null,
    ...
    input8Group.position.set(0.83, 0, -3.5);
    ...
    if (config.get('activationLogic') === 'negative') {
      ...
      if (!negativeMarkerMesh) {
        negativeMarkerMesh = new THREE.Mesh(
          Or8GateVisualFactory.negativeMarkerGeometry,
          Or8GateVisualFactory.negativeMarkerMaterial
        );
