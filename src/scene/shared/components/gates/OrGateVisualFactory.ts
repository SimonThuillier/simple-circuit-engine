...
import type { Component, ComponentState, OrGateState } from 'simple-circuit-engine/core';
 ...
 */
export class OrGateVisualFactory extends ComponentVisualFactoryBase {
  /** Gate high color */
  ...
  createVisual(component: Component): THREE.Object3D {
    ...
    envelopeMaterial.emissive.setHex(OrGateVisualFactory.HIGH_COLOR);
    ...
    input1Group.position.set(0.56, 0, 0.43);
    ...
    input2Group.position.set(0.56, 0, -0.63);
    ...
    if (config.get('activationLogic') === 'negative') {
      ...
      if (!negativeMarkerMesh) {
        negativeMarkerMesh = new THREE.Mesh(
          OrGateVisualFactory.negativeMarkerGeometry,
          OrGateVisualFactory.negativeMarkerMaterial
        );
  ...
  override updateAnimation(object3D: THREE.Object3D, state: ComponentState | null): void {
    ...
    if (!state) {
      if (envelopeMesh.userData.initialState === 'high') {
        ...
        envelopeMesh.material.emissiveIntensity = OrGateVisualFactory.HIGH_INTENSITY;
      } else {
    ...
    const gateState = state as OrGateState;
    if (gateState.isHigh) {
      envelopeMesh.geometry = this.highGeometry;
      envelopeMesh.material.emissiveIntensity = OrGateVisualFactory.HIGH_INTENSITY;
    } else if (gateState.isInTransition) {
      envelopeMesh.geometry = this.transientGeometry;
      envelopeMesh.material.emissiveIntensity = 0.5 * OrGateVisualFactory.HIGH_INTENSITY;
    } else {
      envelopeMesh.geometry = this.lowGeometry;
      envelopeMesh.material.emissiveIntensity = 0;
    }
