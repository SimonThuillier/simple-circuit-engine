...
**Feature**: 3D Circuit Controllers
**Date**: 2025-12-02
...
- Contracts: `CircuitController.ts`, `types.ts`, `ComponentVisualFactory.ts`
...
- Controller contract incomplete (no tool-related methods)
...
**Evidence**:
- FR-021: Controllers "MUST NOT implement mouse/keyboard event listeners"
...
- FR-021: Controllers expose callbacks but "MUST NOT implement mouse/keyboard event listeners"
...
- [ ] T060 [P2] [US2] Implement edit mode toggle in CircuitController
...
Static renderer MUST expose `setActiveTool(toolType)` method for consumers to activate tools programmatically. Controller MUST emit 'toolActivated' event with tool type when activation succeeds.
...
Tools that support multi-step operations (Wire: click source, click target) MUST support cancellation. Controller MUST expose tool cancellation through a consumer-triggered method (e.g., `cancelCurrentToolOperation()`). Wire tool MUST cancel mid-wire operations when cancellation is triggered.
...
Tools MUST delegate all circuit topology modifications to core Circuit API methods. Controller MUST NOT implement circuit logic directly. After successful tool operation, renderer MUST call `update(changedData)` with appropriate delta to refresh visualization.
...
Controller MUST emit the following tool-related events via `on(event, callback)`:
...
When a tool is activated, renderer MUST emit 'cursorChangeRequested' event with cursor type (e.g., 'pointer', 'crosshair', 'move', 'not-allowed'). Controller MUST emit cursor changes during tool operations (e.g., 'not-allowed' when hovering over invalid placement location).
...
**Proposed**: "Static renderer tools (FR-029) MUST perform tool-specific validation (FR-032) before operations. For circuit-specific validation (e.g., pin connection rules, electrical constraints), renderer MUST delegate to core Circuit API methods. Controller MUST NOT implement circuit domain logic."
...
**Current**: "Controllers MUST expose hookable callbacks for the following events via on(event, callback): 'hover', 'unhover', 'position', 'deselect', 'error', 'ready'; renderers MUST NOT implement mouse/keyboard event listeners"

**Analysis**: This requirement is CORRECT and COMPATIBLE with tool system. Consumer implements event listeners, translates to tool method calls on renderer. Controller exposes tool APIs and emits tool events.
...
canvas.addEventListener('click', (e) => {
  ...
  renderer.handleToolClick(worldPos); // Controller exposes tool interaction API
});

canvas.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    renderer.cancelCurrentToolOperation(); // Controller exposes cancellation API
  }
});

// Controller emits events for consumer to react
...
Add to `ICircuitController` interface:
```typescript
/**
...
- [ ] T066 [P2] [US2] Implement tool system architecture in CircuitController
...
| **Contract Completeness** | HIGH | Missing tool APIs | Update types.ts, CircuitController.ts |
...
3. **Contract Update**: Update contracts/types.ts and contracts/CircuitController.ts with tool-related APIs
...
✅ **PASS** - Tools are part of CircuitController module, not core. Clear boundary: tools handle UI interaction, core handles circuit validation.
