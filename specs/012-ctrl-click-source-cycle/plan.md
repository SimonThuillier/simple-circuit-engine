...
Add Ctrl+click interaction to cycle enode sourceType (none → Voltage → Current → none) on branching points and component pins in BuildTool. Visual feedback via color change (white/red/blue) updates immediately. Leverages existing `ENode.source` attribute, `CircuitEditionManager.saveENodeSourceTypeAction()`, and `BranchingPointVisualFactory.updateSourceType()`. Requires extending `ComponentVisualFactory` with pin color updates and relaxing `Circuit.updateENodeSourceType()` to support component pins.
...
└── scene/
    ...
    └── static/
        ...
        └── CircuitEditionManager.ts             # NO CHANGE: saveENodeSourceTypeAction() exists
